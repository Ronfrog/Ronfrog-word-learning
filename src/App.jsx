...
const App = () => {
  ...
  // --------------------------------------------------------------------------
  // API Fetch Utility (Injects Google JWT Token securely to backend)
  // --------------------------------------------------------------------------
  ...
  if (user === null) {
    if (!GOOGLE_CLIENT_ID) {
      return <div className="min-h-screen flex items-center justify-center p-4 bg-slate-50 text-red-500 font-bold">請先在 GCP 環境變數設定 GOOGLE_CLIENT_ID！</div>;
    }
    return (
        ...
        <div className="max-w-5xl mx-auto flex justify-between items-center">
          ...
          <div className="flex gap-2 items-center">
            ...
            <div className="flex items-center gap-2">
               {(user.picture || user.photoURL) ? <img src={user.picture || user.photoURL} className="w-8 h-8 rounded-full border border-slate-200" alt="avatar"/> : <UserCircle size={24} className="text-slate-400" />}
               <button onClick={handleLogout} className="p-2 text-slate-400 hover:text-red-500 rounded-lg hover:bg-red-50" title="登出"><LogOut size={20}/></button>
            </div>
          </div>
        </div>
      </nav>
